...
def tensorConnection (surface : CombinatorialSurface) (n : Nat) : 
  Matrix (Fin n) (Fin n) (StateField p) :=
  fun i j =>
    if i == j then 1 
    else if isGlued surface.ledger i.val j.val then tile.getPhase i.val 
...
theorem holonomic_uniqueness_on_tile 
  ...
  (h_valid1 : Matrix.mulVec (tensorConnection (T := T) (p := p) surface n_bulk) s_bulk1 + Matrix.mulVec D_bdry s_bdry = 0)
  (h_valid2 : Matrix.mulVec (tensorConnection (T := T) (p := p) surface n_bulk) s_bulk2 + Matrix.mulVec D_bdry s_bdry = 0) :
  s_bulk1 = s_bulk2 := by
  let forcing : DirichletForcing p n_bulk n_bdry := {
    D_bulk := tensorConnection (T := T) (p := p) surface n_bulk,
    D_bdry := D_bdry,
    det_nonzero := tensor_laplacian_nonsingular surface h_match
  }
